...
class ProductList(APIView):
    ...
    def get(self, request):
        ...
        if request.GET.get('name') is not None and request.GET.get('name') != "":
          productObj =  Product.objects.all().filter(name__contains = request.GET.get('name'))
        
        # conditioning search stock product
        if request.GET.get('stock') is not None and request.GET.get('stock') != "":
          productObj =  Product.objects.filter(stock__gte = request.GET.get('stock'))
    ...
    def post(self, request):
        ...
        # Check if product name alredy registered
        productObj = Product.objects.filter(name=body['name'])
        if productObj.exists():
            return JsonResponse({
                'error': True,
                'message': "Name product alredy registered"
            })
        else:
